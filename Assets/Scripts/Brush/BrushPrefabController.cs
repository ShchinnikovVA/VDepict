...
public class BrushPrefabController : MonoBehaviour
{
    ...
    public void CreateParticleDot(float _size, GameObject _brushPrefab, Color _color, Sprite _image, float _minDistance)
    {
        ...
        Vector3 _locate = new Vector3(gameObject.transform.position.x, gameObject.transform.position.y, gameObject.transform.position.z); // похици€ в кисти
        ...
        if (Vector3.Distance(_lastPosition, transform.position) > _minDistance) // рассто€ние между спрайтами
        {
            ...
            frameContainer.SetPaintToFrame(_dot); // отправл€ем точку в кадр
            _dot = null;
